...
export function openAssetManager(){
  ...
  const render=()=>{const registry=load(),assets=listAssets(registry);count.textContent=`${assets.length} asset${assets.length===1?'':'s'}`;grid.innerHTML='';if(!assets.length){grid.innerHTML='<div class="asset-empty">No assets yet. Add an image URL to create one.</div>';return}assets.forEach(asset=>{const card=document.createElement('div');card.className='asset-card';card.innerHTML=`<div class="asset-preview">${asset.src?`<img src="${asset.src}" alt="${asset.alt||''}">`:'<span>ASSET</span>'}</div><strong>${asset.name}</strong><code>${asset.path}</code><div class="asset-actions"><button class="asset-use">Use on canvas</button><button class="asset-delete">Delete</button></div>`;card.querySelector('.asset-use').onclick=()=>{window.dispatchEvent(new CustomEvent('webforge:asset-selected',{detail:asset}));root.remove()};card.querySelector('.asset-delete').onclick=()=>{save(removeAsset(load(),asset.id));render()};grid.appendChild(card)})};
